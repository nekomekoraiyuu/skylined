    ...
    if [ "$input_thing" = $'\x1B' ]; then
        ...
        if [ "$input_thing" = "[" ]; then
            ...
            A) # This input this UP ↑ arrow input
                if [ "$selection_option" -gt 1 ]; then
                    selection_option=$(($selection_option - 1))
                fi
                ;;
            B) # This input is DOWN ↓ arrow input
                if [ "$selection_option" -lt "$limit_options" ]; then
                    selection_option=$(($selection_option + 1))
                fi
                ;;
            esac
...
menu_main() {
    ...
    # Bookmark // erase this
    1)
        echo -e "$(echo -e "$main_menu_page_options" | sed "$selection_option s/^/\\\e[32m➔ /" | sed "$selection_option s/$/\\\e[39m/")"
...
versioning_calc() {
    ...
    #####
    if [[ "$versioning" -gt "$VERSION_INFO" && "$first_flag" = "o" ]]; then
        ### Optional flag
    ...
    elif [[ "$versioning" -gt "$VERSION_INFO" && "$first_flag" = "n" ]]; then
        ### Normal flag
    ...
    elif [[ "$versioning" -gt "$VERSION_INFO" && "$first_flag" = "f" ]]; then
...
menu_nsp() {
    ...
    # Check if
    if [[ -z $(ls $SKYLINED_PATH/input/ 2>/dev/null | grep .nsp) ]]; then
    ...
    if [ "$pre_calculated_romdir" = "false" ]; then
        ...
        for i_stuff in $(seq $limit_options); do
...
menu_nsp_update_pick() {
    ...
    if [ -z "$(ls $SKYLINED_PATH/input | grep .nsp | grep -v "$base_selected")" ]; then
    ...
    if [ "$pre_calculated_romdir" = "false" ]; then
        ...
        for i_stuff in $(seq "$limit_options"); do
            ...
            num_order=$(cat $SKYLINED_PATH/temp_stuff/temp_command.sh | grep -n "# Insert here" | cut -d ":" -f 1 | tail -n 1)
            # Then append statement after that line // this command below is insane           add quotation mark#          #make the variable execute																								End mark of sed
            sed -i -e "$num_order a $i_stuff) echo -e $(echo -e '"''$(echo -e ''"$list_test" | sed -e "$selection_option s/^/\\\e[32m➔/" -e "$selection_option s/$/\\\e[39m/")"')\n$(echo -e 'if [ "$INPT_LAST" = "ENTER" ];')\nthen\n$(echo -e 'input_valid="false"')\n$(echo -e 'update_selected=$(echo -e "$list_origin" | sed -n "$selection_option p")')\n$(echo -e 'update_selected_size=$(ls -hl --block-size=M "$SKYLINED_PATH/input/$update_selected" | cut -d " " -f 5 | sed "s/[A-Za-z]//g")')\n$(echo -e 'selection_option=1')\nfi\n;;\n# Insert here" $SKYLINED_PATH/temp_stuff/temp_command.sh
        done
        # At last set precalculation to true since done calculating
        pre_calculated_romdir="true"
        # End if statement
...
menu_nsp_sum() {
    ...
    case $selection_option in
    1)
        echo -e "\e[32m➔ * Yes, proceed to updating.\e[39m\n* No, go back."
        if [ "$INPT_LAST" = "ENTER" ]; then
            INPT_LAST="NULL"
            selection_option=1
        ...
        ;;
    2)
        echo -e "\e[39m* Yes, proceed to updating.\n\e[32m➔ * No, go back.\e[39m"
        if [ "$INPT_LAST" = "ENTER" ]; then
            # A
            input_valid="false"
            selection_option=1
...
menu_update_script() {
    ...
    if [[ $versioning_type = 1 ]] || [[ $versioning_type = 2 ]] || [[ $versioning_type = 3 ]] || [[ $versioning_type = 4 ]]; then
        if [ "$canary_mode" = "true" ]; then
...
if [ "$(cat $CONFIG_DIR/skylined_script.conf 2>/dev/null | grep -h "has_run_skylined_script_once=" | cut -d "=" -f 2)" = "false" ]; then
...
while [ $first_loop = "true" ]; do
    ###### Main menu #######
    # If the selection screen is in default // menu then show main menu // home
    if [ "$selection_screen" = "main" ]; then
        if [ "$INPT_LAST" = "qBACK" ]; then
            exit 0
        fi
        # Show main menu
        menu_main
    ...
    if [ "$selection_screen" = "settings" ]; then
        if [ "$INPT_LAST" = "qBACK" ]; then
            INPT_LAST="NULL"
            selection_screen="main"
            input_valid="false"
            ...
            if [ "$selection_screen" = "script_update" ]; then
                clear
                menu_update_script
                selection_screen="settings"
                sleep 1
    ...
    if [ "$selection_screen" = "about" ]; then
        clear
        menu_about
        sleep 1
    fi
    ...
    while [ $second_loop = true ]; do
        ...
        while [ "$third_loop" = true ]; do
            # If statement (nsp_update_pick)
            if [ "$selection_screen" = "nsp_update_pick" ]; then
                ...
                while [ "$fourth_loop" = "true" ]; do
                    if [ "$selection_screen" = "nsp_update_sum" ]; then
                        menu_nsp_sum
                        if [ "$input_valid" = "true" ]; then
                            read_input_key
                        fi
